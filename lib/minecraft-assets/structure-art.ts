...
export async function getStructureArtSvg(version: string, label: string, preview: StructurePreviewDescriptor) {
  const swatches = await Promise.all(
    preview.blocks.slice(0, 6).map(async (blockId, index) => {
      const layers = await getItemTextureLayers(version, blockId)
      return {
        blockId,
        texture: layers[0] ?? `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(placeholderTile(blockId.replace(/^minecraft:/, ''), index))}`,
      }
    }),
  )
