...
export async function renderEntitySheetCrop(descriptor: CatalogArtDescriptor): Promise<Omit<CatalogArtArtifact, 'path' | 'generatedAt'> & { content: string }> {
  ...
  const cropBox = descriptor.meta?.cropBox
  return {
    format: 'svg',
    ...
    height: 128,
    placeholder: false,
    content: await getEntityArtSvg(version, descriptor.subjectId, label, sourceAsset, cropBox),
  }
}
