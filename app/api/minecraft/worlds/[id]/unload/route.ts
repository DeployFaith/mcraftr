...
export async function POST(
  ...
  if (!bridge.ok || bridge.data.ok === false) {
    return Response.json({ ok: false, error: bridge.ok ? bridge.data.error || 'Failed to unload world' : bridge.error }, { status: 502 })
  }
  const userId = await getSessionUserId(req)
  const serverId = await getSessionActiveServerId(req)
  if (userId) logAudit(userId, 'world_unload', id, undefined, serverId)
